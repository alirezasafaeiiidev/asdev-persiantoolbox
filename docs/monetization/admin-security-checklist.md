...
- [ ] `ADMIN_EMAIL_ALLOWLIST` در production تنظیم و بازبینی شده است.
...
- [ ] query/hash مسیرها قبل از ذخیره‌سازی analytics حذف می‌شوند.
- [ ] metadata analytics فقط از allowlist مجاز عبور می‌کند.
...
- [ ] هر event ingest دارای `metadata.consentGranted=true` است.
- [ ] payloadهای ingest بزرگ‌تر از حد مجاز (200) رد می‌شوند.
