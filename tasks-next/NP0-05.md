...
- Added response-level E2E contracts for CSP/security headers on `/`, `/pdf-tools`, and `/offline`.
- Added nonce/CSP consistency assertion from response HTML to CSP token.
- Updated `proxy.ts` to emit HSTS only in production runtime.
- Added CSP rollout document (`report-only -> enforced`) for staged rollout guidance.

## Verification

- `pnpm playwright test tests/e2e/security-headers.spec.ts --project=chromium --reporter=list`
- `pnpm vitest --run tests/unit/proxy-csp.test.ts`
